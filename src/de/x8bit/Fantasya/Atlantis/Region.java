	...
	public void Init_Handel() {
		...
		for (Paket p : Paket.getPaket("Items")) {
			if (p.Klasse instanceof LuxusGood) {
				__luxus.add(new Nachfrage((Class<? extends Item>) p.Klasse.getClass(), 1.0f));
			}
		}
	}
...
//				new Debug("In " + this + " " + getCoordinates() + " hungern " + hungern + " Bauern, davon sind " + tot + " verhungert.", 0);
	...
	public Region Movement(Richtung richtung, Unit unit) {
		...
		if (!r.istBetretbar(unit)) {
			if (!unit.canFly()) {
				new Fehler(unit + " - " + r + " ist f��r uns nicht zug��nglich.", unit);
				return null;
			}
		}

		// Bewegungspunkte ��berpr��fen
		int bewegungspunkte = (this.hatStrasse(richtung) ? 1 : 2);
