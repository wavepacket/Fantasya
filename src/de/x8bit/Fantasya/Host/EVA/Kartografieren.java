        ...
        for (Partei partei : Partei.getPlayerFactionList()) {
            ...
            for (Unit u : Unit.CACHE) {
                if (u.getOwner() == partei.getNummer()) {
                    partei.addKnownRegion(u.getCoordinates(), true, Unit.class);
                }
            }
        }
    ...
    public void PostAction() {
        ...
        for (Partei partei : Partei.getPlayerFactionList()) {
            ...
            for (RegionsSicht rs : partei.getKnownRegions(false)) {
                ...
                partei.atlas.put(rs.getCoordinates(), rs);
                
                if (rs.hasDetails()) {
                    if (!Unit.CACHE.getAll(r.getCoordinates(),partei.getNummer()).isEmpty()) {
                        // dann auch die Nachbarn mit aufnehmen:
                        for (Coordinates n : rs.getCoordinates().getNeighbours()) {
                            ...
                            if (partei.canAccess(nr)) {
                                ...
                                partei.atlas.put(nrs.getCoordinates(), nrs);
                            }
                        }
                    }
                }
